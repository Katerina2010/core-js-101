...
function extractNameFromTemplate(value) {
  ...
  const lastName = arr[2].split('');
  lastName.pop();
  return `${arr[1]} ${lastName.join('')}`;
}
...
function getRectangleString(width, height) {
  ...
  const rectangle = `${w1}${h.repeat(height - 2)}${w2}`;
  return rectangle;
}
...
function encodeToRot13(str) {
  ...
  return rez.map((el) => el.join('')).join(' ');
}
